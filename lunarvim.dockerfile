...
RUN sudo zypper addrepo https://cli.github.com/packages/rpm/gh-cli.repo -y; \
    ...
    sh rustuio.sh -y; \
    rm rustuio.sh; \
    wget -O lunarvim.sh https://github.com/LunarVim/LunarVim/blob/master/utils/installer/install.sh; \
    sh lunarvim.sh -y; \
    rm lunarvim.sh; \
...
RUN echo "#!/bin/bash" > /usr/bin/init-script; \
    ...
    echo "git clone https://github.com/github/copilot.vim ~/.config/nvim/pack/github/start/copilot.vim" >> /usr/bin/init-script; \
